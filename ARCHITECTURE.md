...
- **RosterStore (Team MVP M3)** — optional store capability (`ReadRoster`/`WriteRoster`) for the synced `team.yaml`. The roster is single-writer by convention (the manager); `internal/sync/tree.go` treats `team.yaml` like `dossier.md`, so a concurrent edit becomes a `sync_concurrent_roster_edit` conflict (sentinel Dossier id `__team_roster__`) instead of being dropped by remote-wins.
