...
**IDs / slugs** (`ids.go`): ULID with prefixes `dos_ art_ sess_ rev_ conf_`. Slug per SPEC §12.2; on creation collision append `-` + last 6 chars of the ULID (Crockford base32). The ID is immutable identity; the canonical slug is mutable through `Service.RenameSlug`. References must use the current canonical slug or immutable ID.

**First-class slug rename** (`FSStore.RenameSlug`): serialize with the new canonical slug, archive the pre-rename revision, atomically replace `dossier.md`, then perform one same-parent `os.Rename` of the complete directory. The operation holds `.sync.lock`, the root namespace lock, and the stable ID lock, rejects canonical/path collisions, and restores the original frontmatter if the directory move fails. Generic `Save`/`Write` reject slug mutation so frontmatter and the directory cannot diverge. Reads resolve only the immutable ID or current canonical slug.
