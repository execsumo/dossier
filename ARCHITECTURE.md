...
**IDs / slugs** (`ids.go`): ULID with prefixes `dos_ art_ sess_ rev_ conf_`. Slug per SPEC §12.2; on creation collision append `-` + last 6 chars of the ULID (Crockford base32). The ID is immutable identity; the canonical slug is mutable through `Service.RenameSlug`, and prior canonical values are persisted as sorted `aliases` in `dossier.md`.

**First-class slug rename** (`FSStore.RenameSlug`): serialize with the new canonical slug and accumulated aliases, archive the pre-rename revision, atomically replace `dossier.md`, then perform one same-parent `os.Rename` of the complete directory. The operation holds `.sync.lock`, the root namespace lock, and the stable ID lock, rejects canonical/alias/path collisions, and restores the original frontmatter if the directory move fails. Generic `Save`/`Write` reject slug mutation so frontmatter and the directory cannot diverge. Reads resolve immutable ID, canonical slug, or any historical alias.
...
**MCP**: use the official Go MCP SDK over stdio. `dossier_rename` is the dedicated optimistic-concurrency surface for canonical slug changes; generic `dossier_update` changes the display name only. Each `dossier_*` tool (SPEC §8.1) is a ~10-line handler: parse input → call one Service method → marshal `Result` into the §8.2 envelope. Map typed errors → §8.2 codes in **one** place (`mcp/errors.go`).
