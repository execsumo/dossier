...
dossier/
  ...
  internal/
    core/                # PURE DOMAIN — no I/O, no third-party deps
      ...
      roster.go          # NormalizeUsername, SanitizeAuthorString, Roster (DisplayName/Has/ResolvePerson/lead-filter scope) — B17
  ...
  assets/                # go:embed — Distillation Guide, context templates, installables
    ...
    pi-extension.ts            # installed into Pi's extensions dir (identity + lifecycle + /spark)
  docs/
...
// (Claude Code reports its whole capability set from ~/.claude.json existing),
// so "first registered harness with LiveSession()" reported a live Pi session
// as Claude Code on any machine with both installed. Adapters answer from
// process evidence (session env vars, the Pi session pointer) via
// harness.ResolveSession; ok=false means no harness owns this process and the
// caller must not guess one. Separate from HarnessRegistry so the lightweight
// test registries supplying only All/Get keep compiling.
...
- **Syncer** — isolates the go-git networking/merge engine (B12 Team Sync) behind one interface. `Service.Sync` orchestrates: it calls the port, then routes any both-modified `dossier.md` into the **existing** `conflicts/*.md` machinery via `store.WriteConflict` (`kind: sync_concurrent_edit`) — one conflict mechanism, two triggers (local edit + cross-machine sync). Remote wins the working tree; the local version is preserved; never last-write-wins, never git merge markers. The merge indexes `dossier.md` files by immutable ID before comparing paths, so a slug-directory rename and a concurrent edit reconcile to one canonical directory rather than producing duplicate copies. Remote wins concurrent different rename targets; losing slugs are not retained. Conflicts leave the queue only through `Service.ResolveConflict`, which saves through the normal write path and then has the store *move* the file to `conflicts/resolved/` (no delete). Unresolved-conflict counts everywhere are derived from `Store.ListConflicts`, never from the last sync run.
- **RosterStore (Team MVP M3)** — optional store capability (`ReadRoster`/`WriteRoster`) for the synced `team.yaml`; the optional `RosterRollbackStore` snapshot/restore capability keeps `team create` transactional when its initial push fails. The roster is single-writer by convention (the manager); `internal/sync/tree.go` treats `team.yaml` like `dossier.md`, so a concurrent edit becomes a `sync_concurrent_roster_edit` conflict (sentinel Dossier id `__team_roster__`) instead of being dropped by remote-wins.
- **Health (P0-9)** — `Service.Health` runs `Doctor` and projects it into a `HealthSummary`; `HealthSummary.Line(now)` is the only formatter, so `doctor`, `sync --status` and the TUI footer cannot disagree. The TUI runs it as an async `tea.Cmd` with a timeout (on start, on fsnotify refresh throttled to once a minute, and on a one-minute tick), and `Syncer.Status` bounds its remote fetch to 5 s.
...
1. Acquire the stable per-dossier advisory lock (`<home>/.locks/<id-hash>.lock`, `flock`). The lock is keyed by immutable ID rather than the movable slug directory.
...
**IDs / slugs** (`ids.go`): ULID with prefixes `dos_ art_ sess_ rev_ conf_`. Slug per SPEC §12.2; on creation collision append `-` + last 6 chars of the ULID (Crockford base32). The ID is immutable identity; the canonical slug and display title are mutable through `Service.Rename`. References use the current canonical slug or immutable ID.

**Rename** (`FSStore.Rename`): serialize the new title and/or canonical slug, archive the pre-rename revision, and atomically replace `dossier.md`. A slug change additionally performs one same-parent `os.Rename` of the complete directory. The operation holds `.sync.lock`, the root namespace lock, and the stable ID lock, rejects canonical/path collisions, and restores the original frontmatter if the directory move fails. Generic `Save`/`Write` reject slug mutation so frontmatter and the directory cannot diverge.
...
- `dossier uninstall [--dir <dir>]` → removes the stable-path binary (Dossier data is untouched).
- `dossier harness uninstall <claude-code|pi>` → removes only Dossier-owned harness config and byte-identical managed assets; unrelated config and user-edited assets are preserved.
...
**Hooks** (SPEC §9): `session-start` builds the library payload (frontmatter scan + capability warnings + guide pointer + active Dossier's distilled state if bound). `session-end`/`pre-compaction` archive the compiled transcript and `Save` a `distilled_state` payload **if the harness supplies one** — none in the registry does, because a hook runs the binary, not the agent, and so cannot distill (`docs/harness-capabilities.md` §3). The field is kept for a harness that could. What this means structurally: the Distilled State is written by in-session `dossier_save` calls, and the boundary is a transcript-archiving backstop, not a save-of-last-resort. Because that is a capability gap rather than a design choice, it degrades visibly per the hard rule — `SessionEnd` samples the revision *before* it writes anything (sampling after would fold in its own transcript artifact and score every session as saved), and when a session persisted nothing it returns a surfaced `Warning` and an `AuditEventDistilledStateNotCaptured` entry rather than the audit-only line it used to leave. A session that did save eagerly takes the quiet path, so the warning stays meaningful. Hook handlers call core; they don't reimplement save.
...
**`PiHarness`** (`internal/harness/pi.go`) detects Pi from its agent directory (`PI_CODING_AGENT_DIR`, default `~/.pi/agent`), a `pi` on PATH, the Pi process environment, or a live session pointer. `Install` writes the embedded `assets/pi-extension.ts` to `<agent dir>/extensions/dossier/index.ts` under the same idempotent/backed-up/confirmed contract (B7/B8) as the Claude Code installer. The same extension bridges Pi's `session_start`/`session_shutdown`/`session_before_compact` events onto the harness-agnostic `dossier hook` CLI, so `Detect` reports the three lifecycle hooks from `PiExtensionInstalled()` — a byte-comparison, so a drifted or pre-bridging extension is not credited with them. It claims **no** MCP: Pi ships no MCP client, and detecting a third-party adapter Dossier neither owns nor tests would be a capability claim it cannot honour. `internal/harness/pisession.go` owns the pointer record, its location, and the depth-bounded process-ancestry walk (procfs, `ps` fallback) that finds the owning Pi process.
...
Per-harness reporting lives in `core`: `HarnessReport` + `Service.HarnessStatus` (read-only detection), `Service.InstallHarness`, and `Service.UninstallHarness` (install/remove one integration by name, the "user added Pi after `init`" path, exposed as `dossier harness list|install|uninstall`). `harnessAdvisories` produces the actionable line that `init`, `harness list` and `doctor` all surface when a harness is installed but its bridge is not. Reporting semantics also live in core, not the printer: `capabilityStatuses` maps the raw booleans to a `CapabilityState` (`available` / `unavailable` / `not applicable` + note), so a capability Dossier deliberately does not use for a harness — Pi's MCP — reads as a design decision with its coverage named, rather than as a broken install the user should go fix. `HarnessReport.IntegrationComplete` is derived from those states (no `unavailable` remaining), which is what lets a surface say "fully functional" without hardcoding per-harness knowledge into an adapter. The boolean map is retained as the machine-readable answer. Doctor treats such an advisory as a warning, not an issue — `Doctor.OK` now tracks `report.Issues`, so a missing integration never masquerades as store damage.
...
**Context assets: embedded is authoritative, `<home>/context/` is a projection.** `guide.md` and `instructions.md` are compiled into the binary and mirrored onto disk to be *read* — the session-start nudge and the suppressed `dossier_session` response both point an agent at those paths, and a user checking what rules the agent follows should be able to open a file rather than unpack a binary. Authority runs the other way: `Store.EnsureContextAssets` rewrites a disk copy that differs from the embedded one and is called from `wire()`, so every command self-heals and an upgraded binary can never keep reading the previous release's Guide (`Init` was the only writer before, making `dossier init` the sole cure). `Store.ReadContextAsset` prefers disk — a deliberate local edit is still honoured — and falls back to the embedded original, so a deleted file degrades to the shipped rules rather than to no rules and no warning. `Doctor` reports drift as an advisory. Two constraints shape the design: refresh **never creates** the context directory, because `team join` clones only into a target holding nothing but `config.yaml` and materializing it there would make joining a team impossible; and the embedded bytes reach core through the Store port, because `TestCorePackageIsPure` forbids `internal/core` importing `dossier/assets`.
...
- **harness**: fixture Claude Code and Pi config dirs; assert `Detect()` capabilities (including that Pi never claims MCP, and claims lifecycle hooks only while the current bundled extension is installed); type-check the embedded `pi-extension.ts` against an installed Pi when the toolchain is present and that `Install()` is idempotent, backs up, and writes nothing without confirmation. Pointer resolution is tested with an injected ancestry function: ancestor pointers resolve, a concurrent session's pointer does not, and malformed/newer-schema pointers are rejected.
