...
func TestMCPUpdateLeadAndStatus(t *testing.T) {
	...
	upd := callTool(t, svc, "dossier_update", `{"id":"dos_1","lead":"Alice","status":"execute"}`)
	if !upd.OK {
	...
	if !strings.Contains(string(raw), "execute") {
		t.Errorf("expected recalled dossier status execute, got %s", raw)
	}

	// Legacy status "waiting" normalizes to "execute".
