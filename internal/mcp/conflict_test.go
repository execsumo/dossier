...
func TestMCPConflictToolsUseStandardEnvelope(t *testing.T) {
	...
	input := strings.Join([]string{
		...
		`{"jsonrpc":"2.0","method":"tools/call","params":{"name":"dossier_resolve_conflict","arguments":{"conflict_id":"conf_mcp","choice":"bad"}},"id":3}`,
		`{"jsonrpc":"2.0","method":"tools/call","params":{"name":"dossier_resolve_conflict","arguments":{"conflict_id":"missing","choice":"keep_shared"}},"id":4}`,
		`{"jsonrpc":"2.0","method":"tools/call","params":{"name":"dossier_resolve_conflict","arguments":{"conflict_id":"conf_mcp","choice":"keep_shared"}},"id":5}`,
	}, "\n") + "\n"
	...
	if len(lines) != 5 {
		t.Fatalf("responses = %d, want 5", len(lines))
	}
	for i, want := range []bool{true, false, false, true} {
		...
		if i == 1 && envelope.Error.Code != ErrCodeInvalidFrontmatter {
			t.Fatalf("invalid choice code = %q", envelope.Error.Code)
		}
		if i == 2 && envelope.Error.Code != ErrCodeNotFound {
