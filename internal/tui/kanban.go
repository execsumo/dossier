...
// 4 lines of screen chrome, the footer, and the search/filter rows when they are
// showing) and subtracts the board's own two header lines, so the footer
// converges on the bottom of the terminal exactly as it does on the dashboard.
func (m Model) kanbanBodyHeight() int {
	headerRows := len(m.activeListRows())
	if headerRows > 0 {
		// Keep the active scope rows clustered, then separate them from the board.
		headerRows++
	}
	h := m.height - 4 - m.footerHeight(ViewKanban) - headerRows - 2
	if h < 3 {
		h = 3
	}
	return h
