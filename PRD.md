...
Pi provides the same *outcomes* through a different route. The original assumption here — that a user-supplied "Claude-like hooks extension" would provide the contract — was verified false against Pi 0.83.0 (ADR 0009): Pi's session environment reaches bash-tool children only, it exposes in-process extension events rather than out-of-process hooks, and it ships no MCP client. Dossier therefore **supplies its own Pi extension**, which publishes session identity and bridges Pi's lifecycle events to the `dossier hook` CLI. So Pi gets deterministic session-start surfacing, end-of-session capture, pre-compaction save, session identity, and transcript access; agent operations run over the CLI rather than MCP. Other harnesses (e.g. Codex, Antigravity) remain out of scope for v1 because they do not currently provide the required capability contract.
