...
| **D3** | Access via **MCP** (auto-surfaces available Dossiers on agent load) **+ CLI/TUI**. **Local-first, optionally team-synced** (amended per B12/ADR 0005, 2026-07-15; originally local, single-user). v1 supports **Claude Code and Pi** (§5.5); Pi is supported through a Dossier-supplied extension, not a user-provided one. | Meet the agent where it lives; degrade gracefully. | Cloud dependency, web app, account system (v1); other harnesses (Codex, Antigravity). |
...
v1 supports **Claude Code and Pi.** Claude Code provides the full capability set: hooks + MCP + transcript capture — full deterministic surfacing, deterministic save backstops, MCP tools, and transcript archive capture.

Pi provides the same *outcomes* through a different route. The original assumption here — that a user-supplied "Claude-like hooks extension" would provide the contract — was verified false against Pi 0.83.0 (ADR 0005): Pi's session environment reaches bash-tool children only, it exposes in-process extension events rather than out-of-process hooks, and it ships no MCP client. Dossier therefore **supplies its own Pi extension**, which publishes session identity and bridges Pi's lifecycle events to the `dossier hook` CLI. So Pi gets deterministic session-start surfacing, end-of-session capture, pre-compaction save, session identity, and transcript access; agent operations run over the CLI rather than MCP. Other harnesses (e.g. Codex, Antigravity) remain out of scope for v1 because they do not currently provide the required capability contract.
...
- **Harness:** v1 supports Claude Code and Pi (§5.5). Claude Code provides the full capability set; Pi reaches the same outcomes through a Dossier-supplied extension, with agent operations over the CLI rather than MCP. If a capability is missing in a given session, Dossier warns rather than silently degrading.
